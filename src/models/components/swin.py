...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        if useBert:
            self.EHR_proj = nn.Sequential(nn.Linear(1024, feature_size*2**4))
        else:
            self.EHR_proj = nn.Sequential(nn.Linear(512, feature_size*2**4))
    ...
    def textEmbedding(self, clin_var, model):
        def generate_prompt(age, weight, chemo, male):
            gender = "male" if male == 1 else "female"
            chemo_status = "has" if chemo == 1 else "has not"
            #change
            #prompt 2
            return f"Computerized tomographies of the head and neck area for a {gender} individual, aged {age:.0f} years, weighing approximately {weight:.0f} kilograms. The {gender} patient has {chemo_status} undergone chemotherapy treatments."
    
        embeddings = []
        for row in clin_var:
            age, weight, male, chemo = row.tolist()
            prompt = generate_prompt(age, weight, male, chemo)
            with torch.no_grad():
                if useBert:
                    ...
                    text_features, _ = torch.max(last_hidden_state, dim=1)
                    # text_features = max_pool.unsqueeze(1)
                    text_features /= text_features.norm(dim=-1, keepdim=True)
                else:
                        text_tokens = clip.tokenize([prompt]).to(device)
                        text_features = model.encode_text(text_tokens)  # Make sure to use the encode_text method
                        text_features /= text_features.norm(dim=-1, keepdim=True)
    ...
    def forward(self, x):
        ...
        img, clin_var = x
        img = img.to("cuda")
        clin_var = clin_var.to("cuda")

        hidden_states_out = self.swinViT(img, self.normalize)
        enc0 = self.encoder1(img)
        ...
        x = hidden_states_out[4].flatten(2).transpose(-1, -2)
        x = x.mean(dim=1, keepdim=True)
        # x = torch.mean(x, dim=1) # CT+PET without EHR
    
        # clin_var = self.textEmbedding(clin_var, model)
        clin_var = clin_var.float() 
        # #clin_var = clin_var.to("cuda")
